...
URL_TEMPLATE = URL_PREFIX + '{{ execution_date.strftime(\'%Y%m\') }}.zip'
ZIP_FILE_TEMPLATE = AIRFLOW_HOME + '/{{ execution_date.strftime(\'%Y%m\') }}.zip'
CSV_FILE_TEMPLATE = AIRFLOW_HOME + '/{{ execution_date.strftime(\'%Y%m\') }}.csv'
PARQUET_FILE_TEMPLATE = AIRFLOW_HOME + '/{{ execution_date.strftime(\'%Y%m\') }}.parquet'
PARQUET_FILE = 'gdelt_{{ execution_date.strftime(\'%Y%m\') }}.parquet'
...
# Monthly data started in January 2006.
...
with DAG(
    dag_id="data_ingest_gdelt_gcs_dag",
